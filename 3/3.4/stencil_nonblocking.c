...
int main(int argc, char** argv) {
  ...
  if (my_id !=comm_size-1 && my_id != 0) {
    ...
    MPI_Isend(&send_value, 1, MPI_INT, next_id, 0, MPI_COMM_WORLD, &request);
    MPI_Irecv(&recv_value, 1, MPI_INT, prev_id, 0, MPI_COMM_WORLD, &request);
  ...
  if (my_id == 0) {
    ...
    MPI_Isend(&send_value, 1, MPI_INT, next_id, 0, MPI_COMM_WORLD, &request);
    MPI_Irecv(&recv_value, 1, MPI_INT, comm_size-1, 0, MPI_COMM_WORLD, &request);
    //MPI_Wait(&request,MPI_STATUS_IGNORE);
  ...
  if (my_id == comm_size-1) {
    ...
    MPI_Isend(&send_value, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &request);
    MPI_Irecv(&recv_value, 1, MPI_INT, prev_id, 0, MPI_COMM_WORLD, &request);
    //MPI_Wait(&request,MPI_STATUS_IGNORE);
